...
parse_file(Path) ->
  case file:open(Path, [read]) of
    {ok, IoDevice} ->
      ...
      {ok, Extra ++ points_of_interest(Tree)};
    {error, Error} ->
      {error, Error}
  end.
...
-spec parse_extra(io:device(), [poi()], erl_anno:location()) ->
   {ok, [poi()]} | {error, any()}.
parse_extra(IoDevice, Acc, StartLocation) ->
  case io:scan_erl_form(IoDevice, "", StartLocation) of
    {ok, Tokens, EndLocation} ->
      case erl_parse:parse_form(Tokens) of
        {ok, Form} ->
          parse_extra(IoDevice, extra(Form, Tokens) ++ Acc, EndLocation);
        {error, _Error} ->
          parse_extra(IoDevice, Acc, EndLocation)
      end;
    {eof, _} ->
      {ok, Acc};
...
-spec extra(erl_parse:abstract_form(), [erl_scan:token()]) -> [poi()].
extra(Form, Tokens) ->
  extra(Form, Tokens, erl_syntax:type(Form)).

-spec extra(erl_parse:abstract_form(), [erl_scan:token()], atom()) -> [poi()].
extra(Form, Tokens, attribute) ->
...
do_find_spec_points_of_interest(Tree, Acc) ->
  case {is_type_application(Tree), Tree} of
    {true, {remote_type, Pos, [Module, Type, Args]}} ->
      Id = {Module, Type, length(Args)},
      POI = els_poi:new(Pos, type_application, Id),
      [POI|Acc];
    {true, {type, Pos, Type, Args}} ->
      Id = {Type, length(Args)},
      POI = els_poi:new(Pos, type_application, Id),
      [POI|Acc];
    {true, {user_type, Pos, Type, Args}} ->
      Id = {Type, length(Args)},
      POI = els_poi:new(Pos, type_application, Id),
      [POI|Acc];
    _ ->
